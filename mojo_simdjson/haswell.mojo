...
fn repeat_until[
    initial_size: Int, //, target_size: Int
](in_: SIMD[DType.uint8, initial_size]) -> SIMD[DType.uint8, target_size]:
    constrained[
        initial_size <= target_size,
        "initial size should be smaller than target size",
    ]()
...
fn classify(in_: SIMD[DType.uint8, 64]) -> JsonCharacterBlock:
    alias whitespace_table = repeat_until[32](
        SIMD[DType.uint8, 16](
            ord(" "),
            100,
            100,
    ...
    alias op_table = repeat_until[32](
        SIMD[DType.uint8, 16](
            0,
            0,
            0,
