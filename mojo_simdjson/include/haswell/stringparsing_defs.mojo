...
struct BackslashAndQuote:
    ...
    @staticmethod
    fn copy_and_find(src: UnsafePointer[UInt8], dst: UnsafePointer[UInt8]) -> BackslashAndQuote:
        # this can read up to 15 bytes beyond the buffer size, but we require
        # SIMDJSON_PADDING of padding
        constrained[SIMDJSON_PADDING >= (Self.BYTES_PROCESSED - 1), "Backslash and quote finder must process fewer than SIMDJSON_PADDING bytes"]()
        v = src.load[width=8]()
